...
def attention_residual_block(input, dilation=1, filters=256):
    ...
    h = tf.keras.layers.Conv2D(filters=filters, kernel_size=1, use_bias=False,
...
def F2M_generator(input_shape=(256, 256, 3)):
    ...
    h = tf.keras.layers.Conv2D(filters=32, kernel_size=7, use_bias=False,
                               kernel_regularizer=l1_l2, activity_regularizer=l1)(h)
    ...
    h = tf.keras.layers.DepthwiseConv2D(kernel_size=1, padding='valid', use_bias=False,
                                        depthwise_regularizer=l1_l2, activity_regularizer=l1)(h)
    h = InstanceNormalization()(h)

    for i in range(6):
        h = attention_residual_block(h, dilation=(i+1)*2, filters=128)
    ...
    for i in range(1):
        h = decode_residual_block(h, dilation=(i+1)*8, filters=64)
